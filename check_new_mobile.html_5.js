...
// ════════════════════════════════════════════════
// BOOKLEGACY V7.1 — FIREBASE + AMIGOS + PWA HOTFIX
...
async function blLoadFirestoreData(){
  ...
  const [booksSnap, librarySnap, sessionsSnap, ratingsSnap, goalsSnap, collectionsSnap] = await Promise.all([
    window.BL.db.collection('books').get(),
    window.BL.db.collection('users').doc(uid).collection('library').get(),
    window.BL.db.collection('users').doc(uid).collection('sessions').get(),
    window.BL.db.collection('users').doc(uid).collection('ratings').get(),
    window.BL.db.collection('users').doc(uid).collection('goals').get(),
    window.BL.db.collection('users').doc(uid).collection('collections').get(),
  ]);
  const raw = {
    uid,
...
function blLegacyBook(item, bookMap){
...
function blAdaptFirestoreToLegacy(raw){
  ...
  const legacyBooks = (raw.library || []).map(item => blLegacyBook(item, bookMap)).filter(l => l.livro);
  const sessoes = (raw.sessions || []).map(s => ({
    id: s.id,
    bookId: s.bookId || '',
  ...
  const notas = (raw.ratings || []).map(r => {
    ...
    return { id:r.id, bookId:r.bookId||'', livro:blTxt(r.title || r.livro), ano:blNum(r.readingYear || r.ano), media, criterios, usuario:'MILTON' };
  }).filter(n => n.livro);
  const metasPorAno = {};
  (raw.goals || []).forEach(g => {
...
function blDecorateHeader(user){
  ...
  el.innerHTML = `${user.photoURL ? `<img src="${user.photoURL}" alt="">` : ''}<span>${user.displayName || 'Usuário'}</span><button type="button" id="blLogout">Sair</button>`;
